...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Function to trigger browser's Find tool (Ctrl + F equivalent)
    function triggerFind() {
        if (window.find) {
            let searchTerm = prompt("Enter text to search in the PDF:");
            if (searchTerm) {
                window.find(searchTerm);
            }
        } else {
            alert("Use Ctrl + F to search in the document.");
        }
    }
